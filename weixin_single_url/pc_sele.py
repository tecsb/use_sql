...
class weixin_spider:
    def __init__(self, keywords):
        ...
        #     self.keywords)
        self.sogou_search_url = keywords
        # 爬虫伪装头部设置
    ...
    def get_selenium_js_html(self, url):
        browser = webdriver.PhantomJS(executable_path=r'D:\Python27\Scripts\phantomjs.exe')
...
if __name__ == '__main__':
    ...
    # f = open('北京长安商场开玩笑-逢节必胖这就是谣言2017年5月2日.html',"w")
    try:
        for i in range(3):
            if i == 1:
                break
    ...
    date = '2017年3月2日原创'
    print date.find('原创')
    print date.replace('原创','')
    # t1 = time.strptime(date, '%Y年%m月%d日')
    date2 = '2017年5月2日'
    t2 = time.strptime(date2, '%Y年%m月%d日')
    # print t1>t2
